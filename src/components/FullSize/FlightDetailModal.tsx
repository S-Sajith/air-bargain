...
import { secondsToHoursMinutes } from "../../common/services/functions";

function FlightDetailModal(props: any) {
  let open = props.open;
  ...
  var departureTime = `${
    deparutreHour < 10 ? `0${deparutreHour}` : deparutreHour
  }:${deparutreMinute < 10 ? `0${deparutreMinute}` : deparutreMinute}`;
  const departureDate = `${
    departureDateTime.getUTCDate() < 10
      ? `0${departureDateTime.getUTCDate()}`
      : departureDateTime.getUTCDate()
  }/${
    departureDateTime.getUTCMonth() + 1 < 10
      ? `0${departureDateTime.getUTCMonth() + 1}`
      : departureDateTime.getUTCMonth() + 1
  }/${departureDateTime.getUTCFullYear().toString().slice(-2)}`;
  ...
  var arrivalTime = `${arrivalHour < 10 ? `0${arrivalHour}` : arrivalHour}:${
    arrivalMinute < 10 ? `0${arrivalMinute}` : arrivalMinute
  }`;
  const arrivalDate = `${
    arrivalDateTime.getUTCDate() < 10
      ? `0${arrivalDateTime.getUTCDate()}`
      : arrivalDateTime.getUTCDate()
  }/${
    arrivalDateTime.getUTCMonth() + 1 < 10
      ? `0${arrivalDateTime.getUTCMonth() + 1}`
      : arrivalDateTime.getUTCMonth() + 1
  }/${arrivalDateTime.getUTCFullYear().toString().slice(-2)}`;
        ...
        <Box sx={style}>
            ...
            <Grid item xs={8}>
              <Typography variant="h6">
                <PanoramaFishEyeRoundedIcon
                  sx={{ fontSize: "1rem", color: "var(--indigo)" }}
                />{" "}
                {departueCity} ({departure})
              </Typography>
              <Typography sx={{ fontSize: "0.8rem", ml: 3, color: "gray" }}>
                {depAirport}
              </Typography>
            </Grid>
          ...
          {routeLength != 0 ? (
            <Grid container sx={{ mt: 2, mb: 1 }}>
              ...
              {index?.map((i) => {
                let arrivalTime: any = new Date(route[i]["local_arrival"]);
                let departureTime: any = new Date(
                  route[i + 1]["local_departure"]
                );
                var diff: any = Math.abs(departureTime - arrivalTime);
                return (
                  <Chip
                    key={i}
                    className="stopover-label"
                    label={`${route[i]["cityTo"]} | ${secondsToHoursMinutes(
                      diff / 1000
                    )}`}
                    sx={{ ml: 1 }}
                  />
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={4}>
              <div
                style={{
                  backgroundColor: props.darkMode ? "#333333" : "#EEEEEE",
                  borderRadius: "0.4em",
                  paddingInline: "5%",
                  paddingBlock: "10%",
                }}
