...
function FlightDeal(props: any) {
  ...
  let departureDate = `${
    day[departureDateTime.getUTCDay()]
  }, ${departureDateTime.getUTCDate()} ${
    month[departureDateTime.getUTCMonth()]
  }`;
  let arrivalTime = `${arrivalHour < 10 ? `0${arrivalHour}` : arrivalHour}:${
  ...
  if (cabin === "M" || cabin === "W") {
    ...
    cabinClass = "Business";
    cabinClassStyle = "upgrade-label";
  } else {
    cabinClass = "First Class";
    cabinClassStyle = "upgrade-label";
  }
  if (flightQuality >= 130) {
    qualityLabel = true;
  } else {
    qualityLabel = false;
  }
  if (route.length === 2) {
    stopovers = `${route.length - 1} Stop`;
  } else if (route.length > 1) {
    stopovers = `${route.length - 1} Stops`;
  } else {
    stopovers = "Non-stop";
  }
      ...
      >
        <Grid container sx={{ paddingInline: "2%" }}>
          ...
          <Grid item xs={6} style={{ textAlign: "right" }}>
            ...
            {qualityLabel ? (
              <>
                &nbsp;{" "}
                <Tooltip title="Flight Quality more than 130">
                  <Chip
                    label="Premium Flying"
        ...
        >
          ...
          <Grid item xs={3}>
            ...
            <Typography
              sx={{ borderTop: "2px dashed #c4c4c4", marginBlock: "2%" }}
            ></Typography>
          ...
          <Grid item xs={3} sx={{ borderLeft: "1.5px solid var(--muted)" }}>
            <Typography
              variant="h5"
              sx={{ fontWeight: "700", color: "#19a096" }}
            >
        ...
        <Grid
          ...
          sx={{
            mt: 2,
            borderTop: "1.2px dashed var(--muted)",
            paddingTop: "2%",
            borderWidth: "0.12rem",
            color: "var(--dark)",
          }}
